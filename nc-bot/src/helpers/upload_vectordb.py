...
def make_connection():
    conn = psycopg2.connect(host="localhost", port=5432, dbname="vectordblab", user="postgres", password="POSTGRES_TEST_123!")
    cur = conn.cursor()
    return cur

def is_existing_collection(cur):
    cur.execute('select exists(select * from information_schema.tables where table_name=%s)', ('langchain_pg_collection',))
...
def retrieve_docs():
    ...
    docs_with_score = vector_store.similarity_search_with_score("What is the regular working hours in Austria?")
...
print(dir_name)
process_data("pdf_data/ED-Time Reporting & Work Time - Finland-071023-193644.pdf", test_delete=False)
retrieve_docs()
#cur = make_connection()
